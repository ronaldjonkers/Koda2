...
class AccountSetupWizard:
    ...
    async def setup_exchange_account(self) -> Optional[dict]:
        ...
        print("  • Exchange server hostname (e.g., exchange.company.com)")
        print("  • Username (often DOMAIN\\username or email)")
        print("  • Password")
        print("  • Email address")
        print()
        
        name = ask("Account name (e.g., 'Work Exchange', 'Company Email')", "Exchange", required=True)
        server = ask("Exchange server hostname", "exchange.company.com", required=True)
